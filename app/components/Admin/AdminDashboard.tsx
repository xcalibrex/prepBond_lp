...
const MetricCard: React.FC<MetricCardProps> = ({ title, value, subtitle, icon, color }) => (
    <div className={`bg-white dark:bg-dark-nav ${DS.radius.card} p-6 border border-gray-100 dark:border-white/5 flex flex-col justify-between h-[180px] transition-all duration-300 hover:-translate-y-1 hover:shadow-xl`}>
        <div className="flex justify-between items-start">
            <div className={`w-10 h-10 flex items-center justify-center ${color} rounded-full`}>
                {icon}
            </div>
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const [
                ...
                // 8. Recent Test Activity (Last 10)
                supabase
                    .from('user_test_sessions')
                    .select('completed_at, total_score, score, profiles:user_id(full_name, email), practice_tests:test_id(title, type)')
        ...
        <div className="flex flex-col gap-8 animate-fade-in-up">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <MetricCard
                    ...
                    color="text-black dark:text-white bg-gray-100 dark:bg-white/10"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className={`bg-white dark:bg-dark-nav ${DS.radius.card} p-8 border border-gray-100 dark:border-white/5`}>
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Recent Test Completions</h3>
                        <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Last 10</span>
                    </div>
                    <div className="space-y-6">
                        {recentActivity.length > 0 ? recentActivity.map((activity, i) => (
                            <div key={i} className="flex items-center gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-[11px] text-gray-500 truncate">
                                        completed {activity.practice_tests?.title || 'Unknown Test'}
                                    </p>
                                </div>
                                <div className="text-right">
                                    <div className="text-sm font-black text-gray-900 dark:text-white">
                                        {activity.total_score !== null ? `${Math.round(activity.total_score)}%` : '-'}
                                    </div>
                            ...
                            <p className="text-sm text-gray-500 text-center py-4">No recent tests taken.</p>
                        )}
                    </div>
                </div>
                <div className={`bg-white dark:bg-dark-nav ${DS.radius.card} p-8 border border-gray-100 dark:border-white/5`}>
