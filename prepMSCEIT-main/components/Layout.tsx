  ...
  <button
    ...
    className={`w-full flex items-center gap-3.5 px-3 py-2.5 text-[13px] font-medium transition-all duration-200 mb-1 mx-auto rounded-lg group relative overflow-hidden
      ${isActive 
        ...
        : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100/50 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-white'
...
export const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange, isDark, toggleTheme, onLogout }) => {
  ...
  return (
    <div className={`h-screen w-screen flex flex-col md:flex-row bg-white text-black dark:bg-black dark:text-white overflow-hidden font-sans relative ${DS.animation.theme}`}>
      
      {/* Mobile Header */}
      {!isFocusMode && (
      <header className={`md:hidden flex items-center justify-between px-6 py-4 bg-white/95 dark:bg-black/95 backdrop-blur-md z-[55] border-b border-gray-100 dark:border-gray-800 shrink-0 h-16 ${DS.animation.theme}`}>
      ...
      {/* Sidebar Drawer */}
      <aside 
        className={`fixed inset-y-0 left-0 z-[60] flex flex-col bg-[#F8F9FD] dark:bg-[#0A0A0A] border-r border-transparent dark:border-white/5 transition-all duration-300 ease-[cubic-bezier(0.25,1,0.5,1)]
      ...
      <main className={`flex-1 w-full h-full relative overflow-y-auto overflow-x-hidden bg-white dark:bg-black ${isSettingsView ? 'md:translate-x-0' : ''} ${DS.animation.theme}`}>
      ...
      {!isFocusMode && (
             <div className={`md:hidden fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-black/95 backdrop-blur-md border-t border-gray-100 dark:border-gray-800 z-[40] px-6 py-2 pb-safe flex justify-between items-center shadow-[0_-4px_20px_rgba(0,0,0,0.05)] transition-all duration-500 ease-in-out ${isSettingsView ? 'translate-y-full opacity-0' : 'translate-y-0 opacity-100'} ${DS.animation.theme}`}>
                {menuItems.map(item => (
                    <button
                        key={item.id}
